...
int main()
{
	...
	while (answer != "exit")
	{
		...
		if (answer == "new")
		{
			...
			if (language == 1)
			{
				cout << "Задайте размер стороны мира (не более 8160)" << endl;
			}
			else
			{
			...
			int sidesize = 542;
			cin >> sidesize;
			while (sidesize > 8160 || sidesize < 1)
			{
				if (language == 1)
				{
		...
		else if (answer == "again")
		{
			...
			else
			{
				...
				if (language == 1)
				{
					cout << "Мир перестроен" << endl;
				}
				else
				{
					cout << "Map is redesigned" << endl;
				}
			}
		}
