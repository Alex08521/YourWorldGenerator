...
{
	public:
		newTerrane(short terr,string name);
		newTerrane();
		~newTerrane();
		...
		void saveTerrane();
		bool loadTerrane();
		void reSetTerrane(short rterr, string rname);
	...
	protected:
		...
		bool constrLand(int X, int Y, int sizeOfContSideX, int sizeOfContSideY, short filler, const char worktype[]);
		bool corrLand();
		bool artifactDeleter();
		bool logicCorr(short a);
		//void spiralGen(short sizeofSide, int localX, int localY, uint8_t filler);
		...
		short typeOfLandVariator(short target, short solid, const char worktype[]);
